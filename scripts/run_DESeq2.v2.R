...
#library(apeglm)
workdir <- "/data/gc-core/yit2001/RNASeq/Project_2023/Project_Granstein-LS-15363_231221/DESeq2_comb2"
countsfile <- paste(workdir, "source", "raw_counts_genes.clean_comb2.txt", sep="/")
...
#colData <- data.frame(condition=factor(c(rep("POST",8),rep("PRE",8))), pair=factor(c(rep(c("726","727","759","768","771","772","781","798"),2))))
colData <- data.frame(condition=factor(c(rep("control",4),rep("Disease",3),rep("control",4),rep("Disease",3))))
...
allconditions <- c("Disease","control")
...
# save R object
save(dds, file=paste(numdir, "dds.Robj", sep="/"))
save(rld, file=paste(numdir, "rld.Robj", sep="/"))
save(vsd, file=paste(numdir, "vsd.Robj", sep="/"))
